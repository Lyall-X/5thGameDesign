Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        /////////////////REED_UE4_ACES_BEGIN/////////////////
        #pragma multi_compile_local _ _HDR_GRADING _TONEMAP_ACES _TONEMAP_NEUTRAL _TONEMAP_XDACES _TONEMAP_T3ACES
        /////////////////REED_UE4_ACES_END/////////////////
        ...
        // #if XRP_AUTO_EXPOSURE
        #pragma multi_compile_local _ _XRP_AUTO_EXPOSURE
        // #endif        
        ...
        // #if _XRP_AUTO_EXPOSURE
        TEXTURE2D(_AutoExposure_Texture);
        // #endif        
        ...
        float4 _Uber_PPOnlyInfo;
		float _Uber_PPDepthPeelingValue;
        ...
        // Uber's input vertex is from -1 to 1 in xy coordinates
        Varyings CropVert(Attributes input)
        {
            float2 size = float2(_Uber_PPOnlyInfo.x, _Uber_PPOnlyInfo.y);
            float2 offset = float2(_Uber_PPOnlyInfo.z, _Uber_PPOnlyInfo.w);
            float2 center = offset + size / 2.0f;
            float2 offset2 = center * 2.0f - 1.0f;

            Varyings output;
            UNITY_SETUP_INSTANCE_ID(input);
            UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(output);
            output.positionCS = TransformObjectToHClip(input.positionOS.xyz * float3(size, 1.0f) + float3(offset2, 0.0f));
            output.uv = input.uv * size + offset;
            return output;
        }
        //Draw only part view vertex shader - End

        half4 BlitFrag(Varyings input) : SV_Target
        {
            input.uv.x = clamp(input.uv.x, 0.0f, 1.0f);
            input.uv.y = clamp(input.uv.y, 0.0f, 1.0f);

            half4 color = SAMPLE_TEXTURE2D_X(_BlitTex, sampler_PointClamp, input.uv).xyzw;
            return color;
        }
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            float exposure = 0.0f;
            {
                exposure = SAMPLE_TEXTURE2D(_AutoExposure_Texture, sampler_PointClamp, float3(0.5f, 0.5f, 0.0f));
            }
            #endif
            ...
            #if _XRP_AUTO_EXPOSURE
            {
                color = ApplyColorGrading(color, exposure, TEXTURE2D_ARGS(_InternalLut, sampler_LinearClamp), LutParams, TEXTURE2D_ARGS(_UserLut, sampler_LinearClamp), UserLutParams, UserLutContribution);
            }
            #else
            {

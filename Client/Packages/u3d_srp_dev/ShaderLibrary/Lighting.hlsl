...
#define UNIVERSAL_LIGHTING_INCLUDED
    ...
    #include "Packages/com.unity.render-pipelines.universal/Runtime/XRPFeatures/XRPShadowMask/XRPShadowMaskUntils.hlsl"
#endif
...
Light GetMainLight(float4 shadowCoord
    #if defined(_XRP_USE_SHADOW_MASK)
        , float shadowmask
    #endif
    #if defined(_XRP_USE_DISTANCE_SHADOWMASK)
        , float shadowLerp
    #endif
)
{
    ...
    float blendLerp;
    #if defined(_XRP_USE_DISTANCE_SHADOWMASK)
        blendLerp = shadowLerp;
    #endif
    
#if defined(_XRP_DECREASE_SHADOWMAP_SAMPLE)
    if (unity_XDShadowData.x < HALF_MIN)
    {
        blendLerp = 1;
    }
    else
    ...
    #if defined(_XRP_USE_SHADOW_MASK)
        light.shadowAttenuation = BlendShadowmask(light.shadowAttenuation, shadowmask, blendLerp);
    #endif
...
#if defined(LIGHTMAP_ON) && defined(_XRP_SHADOWMASK)
    , float4 shadowmask
#endif
#if defined(_XRP_USE_DISTANCE_SHADOWMASK)
    , float shadowLerp
    ...
    half smoothness, half occlusion, half3 emission, half alpha)
{
    ...
    #if defined(_XRP_USE_DISTANCE_SHADOWMASK)
        float shadowLerp = saturate(length(_WorldSpaceCameraPos.xyz - inputData.positionWS) / _MaxShadowDistance);
    #endif
    
    Light mainLight = GetMainLight(inputData.shadowCoord
    #if defined(_XRP_USE_SHADOW_MASK)
        , inputData.bakedAtten.r
    #endif
    #if defined(_XRP_USE_DISTANCE_SHADOWMASK)
        , shadowLerp
    #endif

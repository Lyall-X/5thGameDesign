...
#if defined(_ADDITIONAL_LIGHTS) || defined(_MAIN_LIGHT_SHADOWS_CASCADE) || defined(_XRP_USE_SHADOW_MASK)
...
uniform half _NoLMaxBrightShadow;
uniform half _NoLMinBrightShadow;
uniform half _BrightShadow;
...
real SampleShadowmapFiltered(TEXTURE2D_SHADOW_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData)
{
    ...
    attenuation4 =  SAMPLE_TEXTURE2D_SHADOW(ShadowMap, sampler_ShadowMap, shadowCoord.xyz + samplingData.shadowOffset0.xyz);
...
float3 CustomApplyShadowBias(float3 positionWS, float3 normalWS, float3 lightDirection,float4 CustomShadowBias)
{
    _ShadowBias += CustomShadowBias;
